%%  Decoding choice from many neurons
...
%             h.EdgeColor = [0 0 0];
    ...
    if max(behav.response)==3
        ...
        tab = tab(:,3)/100;
        guess_bpt(3)=sum(tab.*log2(tab));
        g = GLM(behav).setModel('C50-subset').fitCV(numFolds);

    else
        g = GLM(behav).setModel('C50-subset-2AFC').fitCV(numFolds);
    end
    
    %Calculate baseline behavioural model log likelihood (crossval)
    baseline_bpt = mean(log2(g.p_hat));
...
%         ax2(e)=subplot(4,length(behavT.timestamps),e+2*length(behavT.timestamps));
        ...
        bptN=nan(length(spikeTimes),length(tsteps));
        bptNB_NC50=nan(length(spikeTimes),length(tsteps));
        for t = 1:length(tsteps) %for each timestep in that epoch
            queryT = t_zero + tsteps(t);
            
            for neuron = 1:length(spikeTimes) %get activity for all neurons
                firingRate = nan(length(t_zero),1); %num trials
                for trial = 1:length(t_zero)
                    firingRate(trial) =sum(((queryT(trial)-0.05) < spikeTimes{neuron}) & (spikeTimes{neuron} < (queryT(trial)+0.05))) / 0.1;
                end
                firingRate = firingRate - mean(firingRate);
                
                %Test neural model only
                g.data.neur = firingRate;
                g = g.setModel('neur').fitCV(numFolds);
                bptN(neuron,t) = mean(log2(g.p_hat)) - g.guess_bpt;
                
                %Test neural + behav model
                g = g.setModel('C50-subset-neur').fitCV(numFolds);
                bptNB_NC50(neuron,t) = mean(log2(g.p_hat)) - baseline_bpt;
...
for session = 2:(17*2)
    ...
    %Construct design matrix
    X = cfn(behav.contrast_cond,n,c50);
    Y = behav.response;
    ...
    clear F;
    F(length(tsteps),length(behavT.timestamps)) = struct('cdata',[],'colormap',[]);
    for e = 1:length(behavT.timestamps) %for each epoch
        f2=figure('color','w'); aax=axes;
        ...
        for t = 1:length(tsteps) %for each timestep in that epoch
            ...
            neurParams = csvread('C:\Users\Peter\Desktop\GLMNET_DATA\glmnet_params.dat');
            plot(aax,neurParams(:,1),neurParams(:,2),'ko'); title(aax,[behavT.timestamps{e} ' ' num2str(tsteps(t))]);
            axis square;
            xlim([-1 1]*1); xlabel('nL'); ylabel('nR');
            ylim([-1 1]*1); 
...
%             set([ax1(e) ax3(e)],'ytick','','ycolor','w'); ylabel('');
%         end
%         xlabel(ax3(e),behavT.timestamps{e});
    end
    ...
    savefig(gcf,['\\basket.cortexlab.net\home\figures\GLM+NeuralActivity\glmnet_decoders\' dir '\' o{1} '_scatter']);
    clear spikeTimes behav behavT;
end

%% Decoding from widefield data
...
%         ax2(e)=subplot(4,length(behavT.timestamps),e+2*length(behavT.timestamps));
        ...
        for t = 1:length(tsteps) %for each timestep in that epoch
            ...
            ComponentWeights = nan(length(t_zero),numComponents);
            for trial = 1:length(t_zero)
                idx = find(W.t>queryT(trial),1,'first');
                ComponentWeights(trial,:) = W.V(:,idx)';
            end
